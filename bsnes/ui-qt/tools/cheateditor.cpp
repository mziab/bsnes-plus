...
CheatEditorWindow::CheatEditorWindow() {
  ...
  connect(findButton, SIGNAL(clicked(bool)), this, SLOT(findCheatCodes()));
  connect(exportButton, SIGNAL(clicked(bool)), this, SLOT(exportCheatCodes()));
  connect(clearButton, SIGNAL(clicked(bool)), this, SLOT(clearSelected()));
}
...
void CheatEditorWindow::saveForSD2SNES(const char *filename) {
  file fp;
  if(fp.open(filename, file::mode::write)) {
    fp.print("---\n");
    fp.print("# ", notdir(nall::basename(cartridge.baseName)), "\n");
    fp.print("# Generated by ", SNES::Info::Name, " v", SNES::Info::Version, "\n");

    auto items = list->findItems("", Qt::MatchContains);
    foreach(item, items) {
      bool enabled = item->checkState(0);
      string code = item->text(1).toUpper().toUtf8().constData();
      string desc = item->text(2).toUtf8().constData();

      lstring list;
      list.split("+", code.replace(" ", ""));
      ...
      fp.print("  Enabled: ", enabled ? "True" : "False", "\n");
    }
...
void CheatEditorWindow::saveForEverdrive(const char *filename) {
  file fp;
  if(fp.open(filename, file::mode::write)) {
    auto items = list->findItems("", Qt::MatchContains);
    foreach(item, items) {
      ...
      // only GG codes are supported for everdrive
      bool isGG = true;
      for (int i = 0; i < list.size(); i++) {
        const string& t = list[i];
        if(strlen(t) != 9 || t[4] != '-') {
          isGG = false;
          break;
        }
      ...
      if(list.size() == 1) {
        ...
        for (int i = 0; i < list.size(); i++) {
          fp.print(list[i], " ", desc, " (", i+1, " of ", list.size(), ")\n");
        }
      }
    }
...
void CheatEditorWindow::exportCheatCodes() {
  string filename;
  filename << filepath(nall::basename(cartridge.baseName), config().path.cheat);
  filename << ".yml";
  filename = QFileDialog::getSaveFileName(toolsWindow, "Export Cheat Codes",
    filename(), "sd2snes cheats (*.yml);;Super Everdrive cheats (*.txt)").toLocal8Bit().constData();

  if(filename == "") return;
  else if(filename.iendswith(".yml")) saveForSD2SNES(filename);
  else if(filename.iendswith(".txt")) saveForEverdrive(filename);
  else {
    QMessageBox::information(toolsWindow, "Export Cheat Codes",
